...
library(labdsv)
library(mvpart)
library(MVPARTwrap) # Packages MVPARTwrap and rdaTest must be installed from 
				# zip files
...
windows(title="Fish - Chord - UPGMA",12,8)
plot(spe.ch.UPGMA)
...
windows(title="Cophenetic correlation",8,9)
par(mfrow=c(2,2))
...
text(spe.ch.single$height, nrow(spe):2, nrow(spe):2, col="red", cex=0.8)
  # From right to left, this graph shows clear jumps after each fusion between
  # 2 and 6 groups. Go back to the dendogram and cut it at the corresponding distances.
  # Do the groups obtained always make sense? Do you obtain enough groups containing
  # a substantial number of sites?
...
windows(title="Silhouette plot - Ward - K=4")
...
windows(title="Four Ward groups on river")
...
windows(title="Silhouettes - k-means and PAM",12,8)
par(mfrow=c(1,2))
plot(silhouette(spe.kmeans$cluster,spe.ch), main="Silhouette plot - k-means", 
  cex.names=0.8, col= spe.kmeans$cluster+1)
plot(silhouette(spe.ch.pam), main="Silhouette plot - PAM", cex.names=0.8, 
	col=spe.ch.pam$silinfo$widths+1)
...
windows(title="Four k-means groups on river")
...
windows(title="Boxplots of quantitative environmental variables")
par(mfrow=c(2,2))
...
windows(title="Kmeans on species",10,6)
...
spe.pa <- decostand(spe,"pa")      # Transform the data to presence-absence
...
windows(title="0-1 species associations, heat map",14,7)
coldiss(res.pa.dist, nc=10, byrank=TRUE, diag=TRUE)

res.pa.fuz <- fanny(res.pa.dist, k=5, memb.exp=1.5)
summary(res.pa.fuz)
windows(title="0-1 species associations, fuzzy clustering, silhouette plot",8,8)
...
library(mvpart) # If not already loaded
windows(title="Multivariate regression tree - all explanatory variables")
...
# Residuals of MRT
windows(title="Residuals of MRT",10,6)
par(mfrow=c(1,2))
...
windows(title="Fish composition of 4 leaves")
par(mfrow=c(2,2))
for(i in 1:length(groups.mrt)){
...
# MRT as a constrained clustering method for spatial and temporal 
# data sequences

windows(title="MRT with sequential constraint")
...
windows(title="MRT groups on river")
...
# Silhouette plot
windows(title="Fuzzy clustering of fish data - Silhouette plot")
...
windows(title="Fuzzy clustering of fish data - Ordination plot")
